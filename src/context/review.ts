...
const buildingScopeString = (
  ...
  const functionStartLine = scope.line_start;
  const functionEndLine = scope.line_end;
  ...
  hunks.forEach((hunk, idx) => {
    try {
      ...
      if (largestEnclosingFunction) {
        const enclosingRangeKey = `${largestEnclosingFunction.line_start} -> ${largestEnclosingFunction.line_end}`;
